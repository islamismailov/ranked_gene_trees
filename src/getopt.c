...
static const TCHAR *_getopt_initialize (const TCHAR *optstring, struct _getopt_data *d, int posixly_correct)
{
	...
	d->__nextchar = NULL;
	d->__posixly_correct = posixly_correct | !!getenv(_T("POSIXLY_CORRECT"));
...
int _getopt_internal_r (int argc, TCHAR *const *argv, const TCHAR *optstring, const struct option *longopts, int *longind, int long_only, struct _getopt_data *d, int posixly_correct)
{
	...
	if (d->__nextchar == NULL || *d->__nextchar == _T('\0'))
	{
		...
		if (d->optind != argc && !strcmp(argv[d->optind], _T("--")))
		{
			d->optind++;
	...
	if (longopts != NULL && (argv[d->optind][1] == _T('-') || (long_only && (argv[d->optind][2] || !strchr(optstring, argv[d->optind][1])))))
	{
		TCHAR *nameend;
		...
		for (p = longopts, option_index = 0; p->name; p++, option_index++)
			...
			if (ambig && !exact)
			{
				if (print_errors)
				{
					fprintf(stderr, _T("%s: option '%s' is ambiguous\n"),
			...
			if (pfound != NULL)
			{
				...
				if (*nameend)
				{
					...
					else
					{
						if (print_errors)
						{
							...
							{
								fprintf(stderr, _T("%s: option '--%s' doesn't allow an argument\n"),argv[0], pfound->name);
							}
							else
							{
								fprintf(stderr, _T("%s: option '%c%s' doesn't allow an argument\n"),argv[0], argv[d->optind - 1][0],pfound->name);
							}
				...
				else if (pfound->has_arg == 1)
				{
					...
					else
					{
						if (print_errors)
						{
							fprintf(stderr,_T("%s: option '--%s' requires an argument\n"),argv[0], pfound->name);
						}
						d->__nextchar += strlen(d->__nextchar);
						d->optopt = pfound->val;
			...
			if (!long_only || argv[d->optind][1] == _T('-') || strchr(optstring, *d->__nextchar) == NULL)
			{
				if (print_errors)
				{
					...
					{
						/* --option */
						fprintf(stderr, _T("%s: unrecognized option '--%s'\n"),argv[0], d->__nextchar);
					}
					else
					{
						/* +option or -option */
						fprintf(stderr, _T("%s: unrecognized option '%c%s'\n"),argv[0], argv[d->optind][0], d->__nextchar);
					}
				}
...

	{
		...
		TCHAR *temp = (TCHAR*)strchr(optstring, c);
		...
		if (temp == NULL || c == _T(':') || c == _T(';'))
		{
			if (print_errors)
			{
				fprintf(stderr, _T("%s: invalid option -- '%c'\n"), argv[0], c);
			}
			d->optopt = c;
			return _T('?');
		}
		if (temp[0] == _T('W') && temp[1] == _T(';'))
		{
			...
			else if (d->optind == argc)
			{
				if (print_errors)
				{
					fprintf(stderr,	_T("%s: option requires an argument -- '%c'\n"), argv[0], c);
				}
				d->optopt = c;
				if (optstring[0] == _T(':'))
					c = _T(':');
			...
			for (p = longopts, option_index = 0; p->name; p++, option_index++)
				...
				if (ambig && !exact)
				{
					if (print_errors)
					{
						fprintf(stderr, _T("%s: option '-W %s' is ambiguous\n"),
							argv[0], d->optarg);
					}
					d->__nextchar += strlen(d->__nextchar);
					d->optind++;
